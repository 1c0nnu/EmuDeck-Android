 ...
 do
	...
	if [[ $scraper == *"RETROARCH"* ]]; then
		...
		for device_name in ${selected_device_names[@]};
		 do
			 message=$device_name
			 system="${message//'"'/}"            
			 #ls "$romsPathFiltered"/$system
			 echo "$romsPathFiltered"/$system
			 mkdir -p "$romsPathFiltered"/$system/media &> /dev/null
			 mkdir -p "$romsPathFiltered"/$system/media/screenshot &> /dev/null
			 mkdir -p "$romsPathFiltered"/$system/media/box2dfront &> /dev/null
			 mkdir -p "$romsPathFiltered"/$system/media/wheel &> /dev/null
			 ...
			 echo ""
			 #Roms loop
			 for entry in "$romsPathFiltered"/$system/*
			 do
				 #Cleaning up names
				firstString=$entry
				secondString=""
				...
				DIR="$romsPathFiltered"/$system/$romName
				if [ -d "$DIR" ]; then
					startcapture=false
				fi
				...
				if [ $startcapture == true ]; then
					...
					FILE="$romsPathFiltered"/$system/media/screenshot/$romNameNoExtension.png
					if [ -f "$FILE" ]; then
						...
						if [[ $StatusString == *"image/png"* ]] || [[ $StatusString == *"image/jpeg"* ]] || [[ $StatusString == *"image/jpg"* ]]; then
							wget  -q --show-progress "http://thumbnails.libretro.com/$remoteSystem/Named_Snaps/$romNameNoExtension.png" -P "$romsPathFiltered"/$system/media/screenshot/
						else
					...
					FILE="$romsPathFiltered"/$system/media/box2dfront/$romNameNoExtension.png
					if [ -f "$FILE" ]; then
						echo -e "Image already exists, ${YELLOW}ignoring${NONE}" &> /dev/null
					else
						...
						if [[ $StatusString == *"image/png"* ]] || [[ $StatusString == *"image/jpeg"* ]] || [[ $StatusString == *"image/jpg"* ]]; then
							wget  -q --show-progress "http://thumbnails.libretro.com/$remoteSystem/Named_Boxarts/$romNameNoExtension.png" -P "$romsPathFiltered"/$system/media/box2dfront/
							echo -e ""
						else
			  ...
			  do
				...
				romNameNoExtensionForLaunchbox=$(echo $romNameNoExtensionForLaunchbox | sed -r "s/,//g")								
				 ...
				 if [ $startcapture == true ]; then
					...
					FILE="$romsPathFiltered"/$system/media/wheel/$romNameNoExtension.png
					if [ -f "$FILE" ]; then
						 hasWheel=true
					fi
	...
	if [[ $scraper == *"SCREENSCRAPER"* ]]; then
		...
		do			
			...
			metadataFile="$romsPathFiltered"/$system/metadata.pegasus.txt
